...
try
{
    ...
    $contents = Mage::getModel('mobile/order_api')->messenger($orderIncrementId, $orderProtectCode);

    $dir = Mage::getConfig()->getOptions()->getVarDir() . DS . 'draft';

    mkdir($dir, 0777, true);

    $file = sprintf('%s%s%s-%s-%s-%s.txt', $dir, DS, 'order', $orderIncrementId, $orderProtectCode, "messenger");
