...
class Gamuza_Brazil_Model_Mysql4_City extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    const VERSION_REGEX = '/versão=(.*)COD_MUN/';
    ...
    {
        $this->_init ('brazil/city', 'entity_id');
    }

    /**
     * Upload Cities file and import data from it
     */
    public function uploadAndImport (Varien_Object $object)
    {
        if (empty ($_FILES ['groups']['tmp_name']['city']['fields']['import']['value']))
        {
            return $this;
        }

        $csvFile = $_FILES ['groups']['tmp_name']['city']['fields']['import']['value'];
        ...
        if ($headers === false || count ($headers) < 4)
        {
            ...
            Mage::throwException (Mage::helper ('brazil')->__('Invalid Cities File Format'));
        }
        ...
        catch (Exception $e)
        {
            ...
            Mage::logException ($e);
            Mage::throwException (Mage::helper ('brazil')->__('An error occurred while import Cities.') . PHP_EOL . $e->getMessage ());
        }
        ...
        Mage::getSingleton ('adminhtml/session')->addSuccess (Mage::helper ('brazil')->__('The Cities file has been imported successfully!'));

        Mage::getModel ('core/config')->saveConfig (Gamuza_Brazil_Helper_Data::XML_PATH_BRAZIL_CITY_VERSION,  $version);
        Mage::getModel ('core/config')->saveConfig (Gamuza_Brazil_Helper_Data::XML_PATH_BRAZIL_CITY_BEGIN_AT, $beginAt);
        Mage::getModel ('core/config')->saveConfig (Gamuza_Brazil_Helper_Data::XML_PATH_BRAZIL_CITY_END_AT,   $endAt);

        return $this;
    }

    /**
     * Validate row for import and return Cities array or false
    ...
    protected function _getImportRow ($row, $rowNumber, $headers, $version, & $beginAt, & $endAt)
    {
        ...
        if (count ($row) < 4)
        {
            $this->_importErrors [] = Mage::helper ('brazil')->__('Invalid Cities format in the Row #%s', $rowNumber);
...
/*
        ...
        {
            Mage::throwException (Mage::helper ('brazil')->__('Requested Cities table is not valid.'));
        }
*/
